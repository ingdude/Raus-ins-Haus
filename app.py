...
def load_data(sheet_name="Immobilien"):
    try:
        ...
        st.error(f"Datenbank-Fehler bei {sheet_name}: {e}")
        return pd.DataFrame()
    ...
    if not df.empty:
        ...
        for _, r in df.iterrows():
            if r["lat"] and r["lon"]:
                ...
                t_txt = r["Titel"] + (" (PRIVAT)" if r["Privat"] else "")
                folium.CircleMarker([r["lat"], r["lon"]], radius=10, color=color, fill=True, tooltip=t_txt).add_to(m)
                map_points.append({"Titel": t_txt, "Lage": r["Lage"], "lat": r["lat"], "lon": r["lon"]})
...
# --- 📅 BESICHTIGUNGS-KALENDER ---
    ...
    st.write("Terminvorschläge tippen, Häkchen setzen!")
    try:
        df_cal = load_data("Kalender")
        if df_cal.empty: raise ValueError()
    except:
        init_data = {"Terminvorschlag": ["Samstag 10:00", "Sonntag 14:00"]}
        for user in user_liste: init_data[user] = [False, False]
        df_cal = pd.DataFrame(init_data)
    for u in user_liste:
        if u not in df_cal.columns: df_cal[u] = False
        df_cal[u] = df_cal[u].astype(bool)
    edited_df = st.data_editor(df_cal, num_rows="dynamic", use_container_width=True, hide_index=True)
    if st.button("Speichern & Auswerten"):
        save_data(edited_df, "Kalender")
        st.success("Kalender gespeichert!"); st.rerun()
    st.divider()
    st.subheader("🔥 Top Termine")
    gute_termine = []
    for _, r in edited_df.iterrows():
        zusagen = sum([1 for user in user_liste if r.get(user) == True])
        if zusagen >= 2:
            gute_termine.append({"termin": r.get('Terminvorschlag'), "anzahl": zusagen, "wer": ', '.join([u for u in user_liste if r.get(u)])})
    if gute_termine:
        for t in sorted(gute_termine, key=lambda x: x["anzahl"], reverse=True):
    ...
    if not df_links.empty:
        for i, row in df_links.iterrows():
            with st.container(border=True):
                ...
                c1.markdown(f"[{row['URL']}]({row['URL']}) | {row['Beschreibung']}")
...
# --- 🗃️ ARCHIV ---
    ...
    if arch.empty: st.info("Archiv leer.")
